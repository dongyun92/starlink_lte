#!/usr/bin/env python3
"""
실시간 스타링크 모니터링 시스템 - WebSocket 실시간 업데이트 수정됨
"""
...
class RealtimeStarlinkMonitor:
    ...
    def setup_routes(self):
        ...
        @self.app.route('/')
        def dashboard():
            return render_template('starlink_realtime.html')
        ...
        def handle_connect():
            self.logger.info('🔗 WebSocket 클라이언트 연결됨')
            emit('status', {'data': self.latest_data})
        
        @self.socketio.on('disconnect')
        def handle_disconnect():
            self.logger.info('❌ WebSocket 클라이언트 연결 해제됨')
    ...
    def monitoring_loop(self):
        ...
        while self.monitoring_active:
            try:
                ...
                # 🔥 실시간 WebSocket 업데이트 (강제 네임스페이스 지정)
                self.socketio.emit('update', data, namespace='/')
                
                # 로그로 업데이트 확인
                self.logger.info(f"📡 데이터 업데이트: {data['timestamp']}")
...
<body>
    <div class="realtime-indicator" id="realtime-status">🔴 연결 대기</div>
    
    <div class="container">
        <div class="header">
            <h1>🛰️ 스타링크 실시간 모니터</h1>
            <p>WebSocket 실시간 연결 상태 및 성능 모니터링</p>
        </div>
        
        <div class="status-bar">
            <div>
                <strong>API 상태:</strong> <span id="api-status" class="status-good">연결됨</span>
                <span class="connection-status" id="websocket-status">WebSocket: 연결 중...</span>
            </div>
            <div>
        ...
        <div class="controls">
            ...
            <button class="btn" onclick="testWebSocket()">🔧 WebSocket 테스트</button>
        </div>
    ...
    <script>
        // 🔥 명시적으로 포트 9999 지정하여 WebSocket 연결
        ...
        function updateData(data) {
            ...
            // 실시간 표시기 업데이트
            document.getElementById('realtime-status').innerHTML = `🟢 실시간 #${updateCount}`;
        }
        ...
        function testWebSocket() {
            console.log('WebSocket 연결 상태:', socket.connected);
            if (socket.connected) {
                alert('WebSocket 연결됨: ' + socket.id);
            } else {
        ...
        socket.on('connect', function() {
            ...
            document.getElementById('websocket-status').innerHTML = 'WebSocket: ✅ 연결됨';
            document.getElementById('websocket-status').className = 'connection-status';
            document.getElementById('realtime-status').innerHTML = '🟢 실시간 연결';
        });
        
        socket.on('disconnect', function() {
            console.log('❌ WebSocket 연결 해제');
            document.getElementById('websocket-status').innerHTML = 'WebSocket: ❌ 연결 해제';
            document.getElementById('websocket-status').className = 'connection-status disconnected';
            document.getElementById('realtime-status').innerHTML = '🔴 연결 끊김';
        });
        
        socket.on('update', function(data) {
            console.log('📡 실시간 데이터 수신:', data.timestamp);
            updateData(data);
        });
        
        socket.on('status', function(msg) {
            console.log('📊 상태 데이터:', msg);
            if (msg.data) updateData(msg.data);
        });
        
        socket.on('connect_error', function(error) {
            console.error('WebSocket 연결 오류:', error);
            document.getElementById('websocket-status').innerHTML = 'WebSocket: ❌ 연결 오류';
            document.getElementById('websocket-status').className = 'connection-status disconnected';
        });
        
        // 초기화
        document.addEventListener('DOMContentLoaded', function() {
            console.log('🚀 페이지 로드 완료, WebSocket 초기화');
            initChart();
            startMonitoring();
        });
    </script>
...
def create_realtime_system():
    ...
    # HTML 템플릿 저장
    with open('templates/starlink_realtime.html', 'w', encoding='utf-8') as f:
        f.write(HTML_TEMPLATE)
    
    # 모니터링 시스템 실행
    monitor = RealtimeStarlinkMonitor()
    print("🛰️ 스타링크 실시간 모니터링 시스템 (WebSocket 수정됨)")
    print("=" * 70)
    print("📊 대시보드: http://localhost:8080")
    print("🔗 WebSocket: 실시간 업데이트 활성화")
    ...
    print("🔧 WebSocket 디버그 로그 활성화")
    print("=" * 70)
    
    monitor.socketio.run(monitor.app, host='0.0.0.0', port=8080, debug=False)
