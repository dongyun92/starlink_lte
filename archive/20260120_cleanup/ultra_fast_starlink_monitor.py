            ...
            try:
                ...
                if data:
                    ...
                    if self.update_count % 1000 == 0:
                        self.logger.warning(f"🔗 WebSocket 전송: 업데이트 #{self.update_count}, 상태: {data.get('state', 'unknown')}")
    ...
    <script>
        ...
        function updateUltraData(data) {
            ...
            if (data.state === 'CONNECTED') {
                ...
                statusElement.textContent = data.state || 'COLLECTING';
...
• CSV 파일: ${data.csv_file}`);
        ...
        socket.on('ultra_update', function(data) {
            console.log('📡 Ultra Update received:', data);
